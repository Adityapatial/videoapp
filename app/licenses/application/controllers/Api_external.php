...
class Api_external extends REST_Controller
{
  ...
  {     
    ...
    if($api_rate_limit){
      $this->methods['version_post']['limit'] = $api_rate_limit;
      $this->methods['error_post']['limit'] = $api_rate_limit;
      $this->methods['update_post']['limit'] = $api_rate_limit;
      $this->methods['download_size_head']['limit'] = $api_rate_limit;
      $this->methods['download_post']['limit'] = $api_rate_limit;
      $this->methods['verify_post']['limit'] = $api_rate_limit;
  ...
  public function latest_version_post()
  {
    ...
    if(!empty($product_id)){
      ...
      if(!empty($row)){
        ...
        $this->response([
          'status' => false,
          'message' => 'Product ID specified is invalid or product is inactive.'
        ],200);
      }
    }else{
      $this->response([
  ...
  if(!empty($this->post('product_id'))&&!empty($this->post('current_version'))){
    ...
    if(!$product or !$product_active){
      $this->response([
        'status' => false,
        'message'=> 'Product not found or it is inactive, please contact support!',
        'version' => null,
        'changelog'=> null,
        'update_id' => null,
        'has_sql' => null
  ...
  if(!empty($this->post('product_id'))&&!empty($this->post('license_code'))&&!empty($this->post('verify_type'))){
    ...
    if(($type == 'envato')&&empty($this->licenses_model->get_license($license_code)))
    {
      ...
      $response = verify_envato_purchase_code($license_code,$this->user_model->get_config_from_db('envato_username'),$this->user_model->get_config_from_db('envato_api_token'));
