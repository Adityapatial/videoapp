...
class Api_internal extends REST_Controller
{
  ...
  {     
    ...
    if($api_rate_limit){
      $this->methods['test_post']['limit'] = $api_rate_limit;
    }
  ...
  { 
    ...
    if(!empty($product_name)){
        ...
        if(strlen($product_id)<6){
          $this->response([
          'status' => false,
          'message' => "Product ID should be atleast 6 characters long, please check."
          ],400);
        }
        ...
        if($this->db->insert('product_details', $data)){
           ...
           $this->response([
           'status' => true,
           'message' => "New product ".$product_name." with ID ".$product_id." was successfully added."
           ],200);
        }else{
           $this->response([
           'status' => false,
